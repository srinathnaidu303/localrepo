...
for (let select of dropdowns) {
  for (let currc in countryList) {
    ...
    newOption.innerText = currc;
    newOption.value = currc;

    if (select.name === "from" && currc === "USD") {
      newOption.selected = true;
    } else if (select.name === "to" && currc === "INR") {
      newOption.selected = true;
    }
...
btn.addEventListener("click", async (evt) => {
  ...
  const URL = `${Base_Url}/${fromc.value.toLowerCase()}.json`;
  ...
  try {
   ...
   msg.innerText = `${amtVal} ${fromc.value} = ${convertedAmount} ${toc.value}`;


    console.log(`Converted Amount: ${convertedAmount}`);
  } catch (error) {
    console.error("Failed to fetch or convert:", error);
    alert("Conversion failed. Check currency or try again later.");
  }
});
